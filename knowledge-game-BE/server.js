...
app.post("/user/add", (req, res) => {
  let { user_type, user_id, name, email, password, section } = { ...req.body };

  db.query(
    "INSERT INTO users VALUES ('',?,?,?,?,MD5(?),?,'')",
    [user_type, user_id, name, email, password, section],
    (err, result) => {
      if (err) throw err;
      res.send(result);
    }
...
app.post("/answer", (req, res) => {
  let { user_id, level_id, category_id, is_correct, duration_seconds } = {
    ...req.body,
  };
  db.query(
    ...
    (err, result) => {
      ...
      if (result.length <= 0) {
        db.query(
          ...
          [user_id, level_id, category_id, is_correct, duration_seconds],
          (err, result) => {
            if (err) throw err;
